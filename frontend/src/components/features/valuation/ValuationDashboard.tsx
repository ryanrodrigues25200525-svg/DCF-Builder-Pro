...
export function ValuationDashboard({ state, actions, ui, onSearch, onExcelExport, isExporting, initialView, onViewChange }: ValuationDashboardProps) {
  const {
    company,
    historicals,
    financialsNative,
  ...
  return (
    <div className="app-viewport bg-[var(--bg-base)] text-[var(--text-primary)] font-sans overflow-hidden flex">
              ...
              <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                <div className="grid flex-1 grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-4 xl:max-w-[760px]">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className={cn(
                      "mt-1 text-[16px] leading-none font-black uppercase tracking-[0.1em] sm:text-[17px]",
                      "text-white"
                    )}>
        ...
        <AnimatePresence>
          {valuationChange && (
            <motion.div
              ...
              className={cn(
                "fixed top-20 right-6 z-[61] w-[min(360px,calc(100vw-2rem))] rounded-[16px] border px-4 py-3 shadow-[0_20px_50px_rgba(0,0,0,0.28)] backdrop-blur-xl",
                isDarkMode
          ...
          {toast && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0, x: 0 }}
              exit={{ opacity: 0, y: -16, x: 16 }}
              className={`fixed top-20 right-6 z-[60] px-4 py-3 rounded-[12px] flex items-center gap-3 text-[13px] font-medium shadow-lg backdrop-blur-md ${toast.type === 'error'
          ...
          <AnimatePresence mode="wait">
            {loading && !company ? (
              ...
              >
                ...
                <p className="text-xs text-[var(--text-tertiary)] uppercase tracking-wider mt-4">Analyzing financial data...</p>
              </motion.div>
            ) : !company ? (
              <HeroCover
                key="hero"
                  ...
                  {isHealthOpen && (
                    ...
                    >
                      <motion.div
                        ...
                        className="app-health-panel-width h-full bg-[var(--bg-card)] border-l border-[var(--border-default)] shadow-2xl overflow-y-auto custom-scrollbar"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <div className="p-4 border-b border-[var(--border-default)] flex items-center justify-between">
                          <h3 className="text-[13px] font-bold text-[var(--text-primary)] uppercase tracking-wider">Model Health</h3>
                          <button
                            onClick={() => setIsHealthOpen(false)}
