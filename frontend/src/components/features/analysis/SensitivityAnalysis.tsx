...
export const SensitivityAnalysis = memo(function SensitivityAnalysis({ historicals, assumptions, overrides }: Props) {
  ...
  return (
    <div className="sensitivity-theme-scope flex h-full bg-[var(--bg-app)] relative overflow-hidden rounded-2xl border border-[var(--border-default)]">
      <aside className="app-panel-width h-full flex flex-col bg-[var(--bg-sidebar)] relative z-30 border-r border-[var(--border-default)]">
        <div className="flex flex-col px-6 pb-6 border-b border-[var(--border-default)] shrink-0 gap-5 bg-[var(--bg-sidebar)] backdrop-blur-xl z-20 pt-4">
          <div className="flex items-center gap-2">
            <Grid3X3 size={16} className="text-[var(--system-indigo)]" />
            <h4 className="text-[15px] font-black text-[var(--text-primary)] uppercase tracking-[0.2em] opacity-90">Sensitivity Analysis</h4>
          </div>
          <div>
            <p className="text-[12px] font-semibold text-[var(--text-tertiary)] uppercase tracking-[0.12em] mb-2">Current</p>
            <div className="w-full p-3.5 bg-[var(--bg-card)] rounded-xl border border-[var(--border-default)] text-[16px] font-semibold leading-tight text-[var(--text-primary)]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar overflow-x-hidden pt-5 pb-12 [scrollbar-gutter:stable]">
          <div className="px-6 space-y-4">
            {sidebarSections.map((group) => (
              <div key={group.title} className="pb-3 border-b border-[var(--border-default)] last:border-0">
                <p className="text-[14px] font-bold text-[var(--text-secondary)] uppercase tracking-[0.1em] mb-2">{group.emoji} {group.title}</p>
                    ...
                    item.presetId ? (
                      <button
                        ...
                        className={cn(
                          ...
                          presetId === item.presetId ? "bg-[var(--system-blue)]/15 text-[var(--text-primary)]" : "text-[var(--text-secondary)] hover:bg-[var(--bg-glass)]"
                        )}
                      >
            ...
            <button
              ...
              className={cn(
                ...
                isCustom
                  ...
                  : "border-[var(--border-default)] bg-[var(--bg-card)] text-[var(--text-primary)] hover:border-[var(--border-hover)] hover:bg-[var(--bg-glass-hover)]",
            ...
            {isCustom && (
              <div ref={dropdownRootRef} className="space-y-5 rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] p-4">
                <p className="text-[12px] font-black text-[var(--text-tertiary)] uppercase tracking-[0.14em]">Create Custom Sensitivity Matrix</p>
                <label className="block text-[15px] font-semibold text-[var(--text-primary)]">
                  X-Axis Variable
                  <div className="relative mt-1.5">
                    <button
                      type="button"
                      onClick={() => setOpenDropdown((prev) => (prev === "x" ? null : "x"))}
                      className="sensitivity-sidebar-input h-12 w-full rounded-xl border border-[var(--border-default)] bg-[var(--bg-app)] px-4 pr-10 text-left text-[16px] font-semibold text-[var(--text-primary)] outline-none transition-colors hover:border-[var(--border-hover)]"
                    >
                      {axisDefs[customConfig.x].label}
                    </button>
                    <ChevronDown size={18} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[var(--text-tertiary)]" />
                    {openDropdown === "x" && (
                      <div className="absolute z-50 mt-2 max-h-56 w-full overflow-auto rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] p-1 shadow-[0_14px_40px_rgba(0,0,0,0.25)]">
                        {axisOptions.map((option) => (
                          <button
                            ...
                            className={cn(
                              ...
                              customConfig.x === option.value ? "bg-[var(--system-blue)]/20 text-[var(--text-primary)]" : "text-[var(--text-secondary)] hover:bg-[var(--bg-glass)]"
                            )}
                          >
                            {option.label}
                          </button>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <input type="number" value={customConfig.xMin} onChange={(e) => setCustomConfig((prev) => ({ ...prev, xMin: Number(e.target.value) }))} className="sensitivity-sidebar-input h-11 rounded-lg border border-[var(--border-default)] bg-[var(--bg-app)] px-3 text-[15px] text-[var(--text-primary)] outline-none transition-colors focus:border-[var(--system-blue)]/60 focus:ring-2 focus:ring-[var(--system-blue)]/20" />
                  <input type="number" value={customConfig.xMax} onChange={(e) => setCustomConfig((prev) => ({ ...prev, xMax: Number(e.target.value) }))} className="sensitivity-sidebar-input h-11 rounded-lg border border-[var(--border-default)] bg-[var(--bg-app)] px-3 text-[15px] text-[var(--text-primary)] outline-none transition-colors focus:border-[var(--system-blue)]/60 focus:ring-2 focus:ring-[var(--system-blue)]/20" />
                  <input type="number" min={3} max={9} value={customConfig.xSteps} onChange={(e) => setCustomConfig((prev) => ({ ...prev, xSteps: Number(e.target.value) }))} className="sensitivity-sidebar-input h-11 rounded-lg border border-[var(--border-default)] bg-[var(--bg-app)] px-3 text-[15px] text-[var(--text-primary)] outline-none transition-colors focus:border-[var(--system-blue)]/60 focus:ring-2 focus:ring-[var(--system-blue)]/20" />
                </div>
                      ...
                      <div className="absolute z-50 mt-2 max-h-56 w-full overflow-auto rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] p-1 shadow-[0_14px_40px_rgba(0,0,0,0.25)]">
                        {axisOptions.map((option) => (
                          <button
                            ...
                            className={cn(
                              "w-full rounded-lg px-3 py-2.5 text-left text-[14px] font-medium",
                              customConfig.y === option.value ? "bg-[var(--system-blue)]/20 text-[var(--text-primary)]" : "text-[var(--text-secondary)] hover:bg-[var(--bg-glass)]"
                            )}
                          >
                            {option.label}
                          </button>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <input type="number" value={customConfig.yMin} onChange={(e) => setCustomConfig((prev) => ({ ...prev, yMin: Number(e.target.value) }))} className="sensitivity-sidebar-input h-11 rounded-lg border border-[var(--border-default)] bg-[var(--bg-app)] px-3 text-[15px] text-[var(--text-primary)] outline-none transition-colors focus:border-[var(--system-blue)]/60 focus:ring-2 focus:ring-[var(--system-blue)]/20" />
                  <input type="number" value={customConfig.yMax} onChange={(e) => setCustomConfig((prev) => ({ ...prev, yMax: Number(e.target.value) }))} className="sensitivity-sidebar-input h-11 rounded-lg border border-[var(--border-default)] bg-[var(--bg-app)] px-3 text-[15px] text-[var(--text-primary)] outline-none transition-colors focus:border-[var(--system-blue)]/60 focus:ring-2 focus:ring-[var(--system-blue)]/20" />
                  <input type="number" min={3} max={9} value={customConfig.ySteps} onChange={(e) => setCustomConfig((prev) => ({ ...prev, ySteps: Number(e.target.value) }))} className="sensitivity-sidebar-input h-11 rounded-lg border border-[var(--border-default)] bg-[var(--bg-app)] px-3 text-[15px] text-[var(--text-primary)] outline-none transition-colors focus:border-[var(--system-blue)]/60 focus:ring-2 focus:ring-[var(--system-blue)]/20" />
                </div>
                      ...
                      <div className="absolute z-50 mt-2 max-h-56 w-full overflow-auto rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] p-1 shadow-[0_14px_40px_rgba(0,0,0,0.25)]">
                        {outputMetricOptions.map((option) => (
                          <button
                            ...
                            className={cn(
                              "w-full rounded-lg px-3 py-2.5 text-left text-[14px] font-medium",
                              customConfig.outputMetric === option.value ? "bg-[var(--system-blue)]/20 text-[var(--text-primary)]" : "text-[var(--text-secondary)] hover:bg-[var(--bg-glass)]"
                            )}
                          >
                            {option.label}
                          </button>
      ...
      <GlassCard variant="regular" className="sensitivity-panel p-6 bg-[var(--bg-card)] border border-[var(--border-default)]">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
          <div className="p-4 rounded-xl bg-[var(--bg-glass)] border border-[var(--border-default)]">
        ...
        <GlassCard variant="regular" className="sensitivity-panel p-6 bg-[var(--bg-card)] border border-[var(--border-default)]">
          ...
          <div className="space-y-3">
            ...
            {breakEven.map((item) => {
              ...
              return (
                <div key={item.key} className="flex items-center justify-between px-4 py-4 rounded-xl bg-[var(--bg-glass)] border border-[var(--border-default)]">
